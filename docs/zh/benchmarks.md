...
解释 `medium` 和 `large` 时有一个关键点：普通读写、删除、列目录和并发场景保留与 CI 相同的固定操作次数，并且每个场景开始前都会重置数据库。规模变大影响的是 seeded 数据集大小和 seeded 场景重复次数。因此，手动 medium/large artifact 里的普通读写行用于确认同一 workflow 下的正常操作表现；真正对大表规模敏感的是 seeded 行。
