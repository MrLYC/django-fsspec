...
`benchmarks/e2e_test.py` 会针对选中的真实数据库后端验证行为。SQLite 会有意跳过 E2E 并发套件，因为 SQLite 写入是串行化的；MySQL、PostgreSQL 和 Oracle 会在 CI 中运行完整并发场景。

E2E 覆盖这些面向用户的工作流：

| 领域 | 覆盖内容 |
|------|----------|
| 核心文件 API | 写入、读取、覆盖、空文件、不同文件大小和 block layout、多块文件、范围读取、校验和验证 |
...
|----------|--------|-----------|---------------|-----------|----------|
| 小/中文件写入 | 3.85-3.90ms | 7.13-11.20ms | 5.84-5.89ms | 6.87-7.55ms | 大多数服务端路径的小文件写入应保持个位数毫秒；本次 run 中 MySQL 8.0 / Django 5.2 的写入路径更慢。 |
| 大文件写入，1 MB | 11.65ms | 45.30-71.00ms | 36.16-37.15ms | 37.18ms | 大文件写入主要受 block 持久化影响；GitHub Actions 的网络数据库上应预期几十毫秒。 |
| 读取与 seek | 1.26-2.25ms | 2.33-5.24ms | 2.41-9.29ms | 3.02-12.81ms | 读取通常是个位数毫秒，但 1MB 读取会体现后端差异和单次 run 波动。 |
| 缓存读取 | 0.12-0.24ms | 0.10-0.46ms | 0.11-0.53ms | 0.12-0.59ms | 热本地缓存路径主要体现 fsspec 缓存开销，而不是数据库延迟。 |
| 覆盖/删除 | 3.05-5.17ms | 6.39-13.67ms | 4.78-8.78ms | 5.66-9.99ms | 修改已有路径通常处在个位数到低两位数毫秒。 |
| 目录列举 | 3.71-3.97ms | 5.35-7.01ms | 5.92-6.39ms | 5.84-8.50ms | 有界的平铺目录和浅层嵌套目录列举，在各服务端数据库上表现接近。 |
| 8 线程并发 | 只读 2.08ms；写密集锁冲突 | 3.06-9.90ms | 2.24-6.32ms | 3.32-7.61ms | 并发写入应使用服务端数据库。SQLite 适合本地或读多写少场景，但写密集并发会遇到 `database is locked`。 |
...
这些结果里的后端适配判断比较明确：SQLite 适合单元测试、本地开发和低并发部署；存在并发写入时应使用 MySQL、PostgreSQL 或 Oracle。PostgreSQL 在最新 CI run 和已测递归 `find` 负载上最均衡。Oracle 在手动铺底 artifact 的根目录列举和 `exists` 上仍然表现较好，但最新 CI 的 1MB 读写慢于上一次文档记录的 run。MySQL 仍然可用，但本组数据里大文件写入和递归 `find` 相对更慢。

Django 版本会带来可见差异，但不是主导因素。在最新 CI run 中，MySQL 8.0 使用 Django 5.2 时，整张测量表平均延迟比 Django 4.2 高约 16.7%，主要来自写入密集场景。PostgreSQL 16 使用 Django 5.2 时平均延迟低约 0.3%，但单个场景既有轻微退化，也有小幅改善。
...
CI 规模数据来自 2026-07-13 的成功 GitHub Actions CI run [`29259244795`](https://github.com/MrLYC/django-fsspec/actions/runs/29259244795)，commit 为 `eb8fbc2`。该 run 使用当前默认 32KB 块大小，每个 CI artifact 都记录为 `block_size: 32768`。Medium 和 large 铺底数据来自 2026-06-29 手动触发的 Large Benchmark run。这些铺底 run 是目前最新可用的大表参考 artifact；应按规模和场景比较，不应视为与最新 CI 小规模表来自同一个 commit。

| Artifact | Run | Commit | 范围 |
|----------|-----|--------|------|
| `benchmark-sqlite` | [29259244795](https://github.com/MrLYC/django-fsspec/actions/runs/29259244795) | `eb8fbc2` | CI 规模，SQLite |
| `benchmark-mysql-8.0-django-4.2` | [29259244795](https://github.com/MrLYC/django-fsspec/actions/runs/29259244795) | `eb8fbc2` | CI 规模，MySQL 8.0 + Django 4.2 |
| `benchmark-mysql-8.0-django-5.2` | [29259244795](https://github.com/MrLYC/django-fsspec/actions/runs/29259244795) | `eb8fbc2` | CI 规模，MySQL 8.0 + Django 5.2 |
| `benchmark-postgres-16-django-4.2` | [29259244795](https://github.com/MrLYC/django-fsspec/actions/runs/29259244795) | `eb8fbc2` | CI 规模，PostgreSQL 16 + Django 4.2 |
| `benchmark-postgres-16-django-5.2` | [29259244795](https://github.com/MrLYC/django-fsspec/actions/runs/29259244795) | `eb8fbc2` | CI 规模，PostgreSQL 16 + Django 5.2 |
| `benchmark-oracle` | [29259244795](https://github.com/MrLYC/django-fsspec/actions/runs/29259244795) | `eb8fbc2` | CI 规模，Oracle 23 |
